...
- Config cleanup in progress: Pydantic v2 SqlTarget model expanded (instance, connect_timeout, IP metadata, auth alias for integrated/sql, credential refs optional); config repository now uses JSONC comment stripping and normalizes credential refs. Legacy `.example.json` configs removed; JSONC examples are canonical.
- Config schemas added: JSON Schemas under `config/schemas/` (sql_targets, audit_config, credential). Docs at `docs/config/SCHEMA.md`; examples updated (`config/sql_targets.example.jsonc`, `config/credentials/os_admin.example.jsonc`, README links).
