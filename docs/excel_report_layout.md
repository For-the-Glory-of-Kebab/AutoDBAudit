...
> **20 Sheets** generated by `python main.py --audit`
...
| 8 | Client Protocols | `client_protocols.py` | Network protocol audit (NEW) |
| 9 | Databases | `databases.py` | Database properties |
| 10 | Database Users | `db_users.py` | Per-database users |
| 11 | Database Roles | `db_roles.py` | Role memberships |
| 12 | Role Matrix | `role_matrix.py` | Visual grid of user roles |
| 13 | Permission Grants | `permissions.py` | Explicit GRANT/DENY audit |
| 14 | Orphaned Users | `orphaned_users.py` | Users without logins |
| 15 | Linked Servers | `linked_servers.py` | Linked server config |
| 16 | Triggers | `triggers.py` | Server/database triggers |
| 17 | Backups | `backups.py` | Backup status |
| 18 | Audit Settings | `audit_settings.py` | Audit configuration |
| 19 | Encryption | `encryption.py` | SMK/DMK/TDE status |
| 20 | Actions | `actions.py` | Remediation tracking |
