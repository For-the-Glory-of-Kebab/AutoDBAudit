...
| K | Clustered | BOOL | `✓`/`✗` Is Clustered. |
| L | HADR | BOOL | `✓`/`✗` Has Availability Groups enabled. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if attention needed (non-compliant). |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Status | STATUS | `PASS`/`FAIL`/`WARN` compliance status. |
| E | Is Disabled | BOOL | `✓`/`✗` Is account disabled? |
| F | Is Renamed | BOOL | `✓`/`✗` Is account renamed (not 'sa')? |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if non-compliant. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Login Name | TEXT | Login Name. |
| E | Login Type | TEXT | SQL Login / Windows User / Windows Group. |
| F | Enabled | BOOL | `✓ Yes` / `✗ No`. |
| G | Password Policy | BOOL | `✓ Yes` / `✗ No` (N/A for Windows). |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if sysadmin member needs justification. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if non-compliant/needs review. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Service Name | TEXT | Service Name (e.g., SQL Server Agent). |
| E | Type | TEXT | Service Type (e.g., Database Engine). |
| F | Status | STATUS | `✓ Running` / `✗ Stopped`. |
| G | Startup | STATUS | `⚡ Auto` / `🔧 Manual` / `⛔ Disabled`. |
| H | Service Account | TEXT | Account running the service. |
| I | Compliant | BOOL | `✓` / `✗` (based on account & type check). |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if `TRUSTWORTHY ON` (User DB). |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Database | TEXT | Database Name. |
| E | Owner | TEXT | Database Owner. |
| F | Recovery | STATUS | `🛡️ Full` / `📦 Bulk` / `⚡ Simple`. |
| G | State | STATUS | `✓ Online` / `⛔ Offline` / etc. |
| H | Data (MB) | SIZE | Data file size. |
| I | Log (MB) | SIZE | Log file size. |
| J | Trustworthy | BOOL | `✓ ON` (Bad for User DB) / `✗ OFF`. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if Guest enabled or Orphaned found. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Database | TEXT | Database Name. |
...
| H | Login Status | STATUS | `✓ Mapped` / `⚠️ Orphaned` / `🔧 System`. |
| I | Compliant | STATUS | `✓` / `⚠️ Review` / `❌ GUEST`. |
| J | Review Status | STATUS | User review dropdown. |
| K | Justification | MANUAL | Justification. |
| L | Last Reviewed | DATE | Date of review. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if High Risk role assigned. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Database | TEXT | Database Name. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` Always (since all represent issues). |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Database | TEXT | Database Name. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if High Risk permission. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
...
| F | db_owner | BOOL | `✓`/`👑 YES` (if not dbo). |
| G | db_securityadmin| BOOL | `✓` if member. |
| H | db_accessadmin | BOOL | `✓` if member. |
| I | db_backupoperator| BOOL | `✓` if member. |
| J | db_ddladmin | BOOL | `✓` if member. |
| K | db_datareader | BOOL | `✓` if member. |
| L | db_datawriter | BOOL | `✓` if member. |
| M | db_denydatareader | BOOL | `✓` if member. |
| N | db_denydatawriter | BOOL | `✓` if member. |
| O | Other Roles | TEXT | Comma-separated custom roles. |
| P | Risk | STATUS | `🔴 High` if db_owner (non-dbo). |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if security issue detected. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if missing backup. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Database | TEXT | Database Name. |
...
| J | Status | STATUS | `✅ OK` / `❌ Missing` / `⚠️ Old`. |
| K | Review Status | STATUS | User review dropdown. |
| L | Justification | MANUAL | Justification. |
| M | Last Reviewed | DATE | Date of review. |
...
| Col | Header | Type | Description |
| :--- | :--- | :--- | :--- |
| A | (Action) | ACTION | `⏳` if discrepant protocol enabled. |
| B | Server | TEXT | Server Name. |
| C | Instance | TEXT | Instance Name. |
| D | Protocol | TEXT | Protocol Name (TCP/IP, Named Pipes). |
| E | Enabled | BOOL | `✓ Yes` / `✗ No`. |
| F | Port | TEXT | Port Number (if TCP). |
| G | Status | STATUS | `✅ Compliant` / `⚠️ Needs Review`. |
...
| H | Backup Status | STATUS | `✓ Backed Up` / `⚠️ Not Backed Up`. |
...
| H | Change Type | STATUS | `✓ Closed` (Fix) / `⏳ Open` (Regressed). |
