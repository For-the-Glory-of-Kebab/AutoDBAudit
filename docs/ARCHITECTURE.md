...
### 4. Action Sheet Strategy
The Action Sheet uses a **Diff-Log** strategy (`SyncService`):
- **Source of Truth**: `action_log` table in SQLite.
- **Persistence**: "First Detected" timestamps are preserved.
- **User Override**: Manual edits in Excel (dates/notes) are prioritized and preserved during sync.
- **Scope**: Tracks status transitions (Fixed, Regression, New) rather than snapshotting state.
