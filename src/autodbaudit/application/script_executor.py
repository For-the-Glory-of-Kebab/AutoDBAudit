...
class ScriptExecutor:
    ...
    def __init__(
        ...
        self._targets_cache: dict | None = None
        ...
        if self._targets_cache is None:
            if self.targets_file.exists():
                with open(self.targets_file, "r", encoding="utf-8") as f:
                    self._targets_cache = json.load(f)
            else:
                self._targets_cache = {"targets": []}
        return self._targets_cache
    
    def _get_connection_for_script(self, script_path: Path) -> tuple[str, str, str | None]:
        """
        ...
        for target in targets.get("targets", []):
            if target.get("host") == server:
                if target.get("windows_auth"):
                    connection_login = "WINDOWS_AUTH"
                else:
                    connection_login = target.get("username", "sa")
                break
    ...
    def _check_credential_safety(
        ...
        alter_login_pattern = re.compile(
            r"ALTER\s+LOGIN\s+\[?" + re.escape(connection_login) + r"\]?",
            re.IGNORECASE
        )
        drop_login_pattern = re.compile(
            r"DROP\s+LOGIN\s+\[?" + re.escape(connection_login) + r"\]?",
            re.IGNORECASE
        )
        
        for i, batch in enumerate(batches):
            if alter_login_pattern.search(batch) or drop_login_pattern.search(batch):
                warnings.append(
        ...
        if connection_login.lower() == "sa":
            ...
            rename_pattern = re.compile(r"WITH\s+NAME\s*=", re.IGNORECASE)
            
            for i, batch in enumerate(batches):
                if sa_pattern.search(batch) and i not in skip_indices:
                    warnings.append(
    ...
    def execute_folder(
        ...
        if rollback:
            ...
            scripts = sorted(
                s for s in folder.glob("*.sql") 
                if "_ROLLBACK" not in s.name
            )
        ...
        print(f"Scripts: {len(results)} total, {total_success} successful, {total_failed} with errors")
        
        for result in results:
            status = "✓" if result.success else "✗"
            print(f"  {status} {result.script_path.name}: {result.successful}/{result.total_batches} batches")
    ...
    def execute_script(
        ...
        # Get connection info
        server, instance, connection_login = self._get_connection_for_script(script_path)
        ...
        warnings, skip_indices = self._check_credential_safety(batches, connection_login)
        ...
        for i, batch in enumerate(batches):
            if i in skip_indices:
                result.skipped += 1
                result.batch_results.append(BatchResult(
                    batch_num=i + 1,
                    success=True,
    ...
    def _connect(self, server: str, instance: str):
        ...
        if target.get("windows_auth"):
            ...
            username = target.get("username", "sa")
...
def main():
    ...
    parser.add_argument("--rollback", action="store_true", help="Execute rollback scripts")
    ...
    if args.folder:
        executor.execute_folder(args.folder, dry_run=args.dry_run, rollback=args.rollback)
    elif args.script:
