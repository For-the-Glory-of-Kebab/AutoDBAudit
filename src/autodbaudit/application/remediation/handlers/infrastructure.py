...
.DESCRIPTION
    Checks and optionally remediates Windows-level settings that T-SQL cannot reach.
    - Services (SQL Browser, Agent, CEIP)
    - Client Protocols (TCP, Named Pipes, Shared Memory)
    - Firewall Ports ({self.ctx.port}) and IP Listeners
    - Windows Power Plan
...
# 1.1 SQL Browser (Should be DISABLED unless named instances used - Standard Hardening)
...
# 1.2 SQL Telemetry (CEIP) - Often unwanted
...
# 2. Client Protocols (Registry Hack method for portability)
Write-Host "`n[2] Checking Client Protocols..."
# Note: Paths vary by version (v16 = 2022, v15 = 2019). Attempting generic lookup or specific.
# Simplified: Looking for known instance path.
...
if ($InstanceId) {{
    ...
    # 2.2 Disable VIA (Sm - actually Shared Memory is Sm, Via is distinct usually legacy)
    # Keeping Shared Memory (Sm) Enabled usually for local, TCP Enabled for remote.
...
Write-Host "  [INFO] Detected Domain Facing IPs: $($IPs -join ', ')"

$rule = Get-NetFirewallRule | Where-Object {{ $_.DisplayName -like "*SQL*$Port*" }}
if (-not $rule) {{
    Write-Host "  [WARN] No explicit firewall rule found for SQL Port $Port" -ForegroundColor Yellow
    if ($ApplyFix -or $Aggressiveness -ge 3) {{
         # Level 3 only: Auto-create firewall rule
         Write-Host "  [FIX] Creating Firewall Rule..."
         New-NetFirewallRule -DisplayName "SQL Server Port $Port" -Direction Inbound -LocalPort $Port -Protocol TCP -Action Allow | Out-Null
...
$plan = Get-CimInstance -ClassName Win32_PowerPlan -Namespace root\\cimv2\\power | Where-Object {{ $_.IsActive }}
if ($plan.ElementName -notlike "*High Performance*") {{
    ...
    if ($ApplyFix -or $Aggressiveness -ge 2) {{
         ...
         $highPerf = Get-CimInstance -ClassName Win32_PowerPlan -Namespace root\\cimv2\\power | Where-Object {{ $_.ElementName -like "*High Performance*" }}
         if ($highPerf) {{
