...
class ExcelAnnotationReader:
    ...
    - Server, Instance (for entity key)
    - Notes, Reason (user-editable)
    ...
    def read_annotations(self) -> Iterator[ExcelAnnotation]:
        ...
        for sheet_name, config in self.SHEET_CONFIG.items():
            ...
            if entity_idx is None:
                logger.debug("Sheet %s missing entity column %s", sheet_name, entity_col)
                continue
...
class ExceptionService:
    ...
    Usage:
        service = ExceptionService(db_path, excel_path)
        count = service.apply_exceptions()
    """
    ...
    def apply_exceptions(self, audit_run_id: int | None = None) -> dict:
        ...
        if audit_run_id is None:
            ...
            audit_run_id = row["id"] if row else None
        ...
        for annotation in reader.read_annotations():
            try:
                ...
                counts["errors"] += 1
                logger.warning("Failed to apply annotation for %s: %s", 
                             annotation.entity_key, e)
        
        conn.close()
        
        logger.info(
            "Apply exceptions complete: %d applied, %d errors",
            counts["applied"], counts["errors"]
        )
    ...
    def get_exceptions_for_entity(self, entity_key: str) -> dict:
        ...
        for row in rows:
            ...
            if row["status_override"]:
                result["status_override"] = row["status_override"]
