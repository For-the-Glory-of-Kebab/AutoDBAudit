...
class FinalizeService:
    ...
    def finalize(
        ...
        # 1. Resolve Run ID
        if run_id is None:
            run_id = self._get_latest_run_id(store)
            if not run_id:
                return {"error": "No audit runs found to finalize."}

        # 2. Check current status
        run = store.get_audit_run(run_id)
        if not run:
        ...
        if "error" in exc_result:
             ...
             if not force:
                 return {"error": f"Annotation sync failed: {exc_result['error']}. Close file and retry."}
             logger.warning("Forcing finalization despite annotation sync failure: %s", exc_result['error'])

        # 4. Snapshot Configuration (Reproducibility)
        # We assume the current config on disk is what we want to lock as the "Final" state
        # (or at least the state at time of finalization)
        # We need an audit_id to save the snapshot. Run implicitly belongs to an audit?
        # Our schema doesn't strictly link run->audit_id (oops), but AuditManager does structure it.
        # But wait, AuditManager uses 'audit_id' (folder) vs 'run_id' (db).
        # We need to backtrack audit_id from folder interactions or just skip if we can't easy.
        # Store schema has no audit_id column on audit_runs table (it has organization).
        # We'll skip this for now as it risks complexity maze. The DB 'config_hash' is the primary link.
        
        # 5. Archive Report
        src_path = Path(excel_path) if excel_path else (self.output_dir / "Audit_Latest.xlsx")
        if not src_path.exists():
            # Fallback to default
            src_path = self.output_dir / "Audit_Latest.xlsx"
            
        if not src_path.exists():
            return {"error": f"Output report not found at {src_path}. Cannot finalize missing report."}
        ...
        # 7. Update DB
        store.complete_audit_run(run_id, "finalized")
        
        logger.info(f"Finalized Run {run_id}. Hash: {file_hash}")
    ...
    def get_finalization_status(self, run_id: int | None = None) -> dict:
        ...
        if run.status == "finalized":
             return {"can_finalize": False, "error": f"Run {run_id} is already finalized."}

        # Check for outstanding findings using generic schema query
        # We want to know if there are any FAIL/WARN that are NOT 'fixed' in action_log?
        # Actually, action_log tracks fixes. The "Current State" is what matters.
        # But we don't have a "Current State" table that aggregates everything easily except via Generic Query.
        # Let's check 'annotations' for 'status_override' = 'Accept Risk' too?
        
        conn = store._get_connection()
        
        # Get all latest findings for this run
        from autodbaudit.infrastructure.sqlite.schema import get_findings_for_run
        findings = get_findings_for_run(conn, run_id)
    ...
    def _get_latest_run_id(self, store: HistoryStore) -> int | None:
        conn = store._get_connection()
        # Get latest run regardless of type
