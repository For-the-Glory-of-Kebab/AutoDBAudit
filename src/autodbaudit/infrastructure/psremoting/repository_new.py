...
class PSRemotingRepository:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                (server_name, connection_method, auth_method, successful,
                 ...
                 sql_targets, baseline_state, current_state, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                profile.server_name,
    ...
    def update_connection_profile_success(self, server_name: str, auth_method: str, connection_method: ConnectionMethod) -> None:
        """
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                SET auth_method = ?, connection_method = ?, successful = 1,
                    last_successful_attempt = ?, last_attempt = ?, attempt_count = attempt_count + 1,
                    updated_at = ?
                WHERE server_name = ?
    ...
    def log_connection_attempt(self, attempt: ConnectionAttempt) -> int:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                (profile_id, attempt_timestamp, layer, connection_method, auth_method,
                 success, error_message, duration_ms, config_changes, rollback_actions,
                 manual_script_path, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                attempt.profile_id,
    ...
    def get_recent_attempts(self, server_name: str, limit: int = 10) -> List[ConnectionAttempt]:
        """
        ...
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT a.id, a.profile_id, a.attempt_timestamp, a.layer, a.connection_method,
                       a.auth_method, a.success, a.error_message, a.duration_ms,
                       a.config_changes, a.rollback_actions, a.manual_script_path, a.created_at
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                (profile_id, state_type, collected_at, winrm_service_status,
                 winrm_service_startup, winrm_service_account, winrm_firewall_enabled,
                 winrm_firewall_ports, trusted_hosts, network_category,
                 local_account_token_filter, allow_unencrypted, auth_basic,
                 auth_kerberos, auth_negotiate, auth_certificate, auth_credssp,
                 auth_digest, execution_policy, enable_lua, remote_management_enabled,
                 full_state_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                state.profile_id,
